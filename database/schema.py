...
def create_video_header_table(table_name: str):
    videos_header_create_table_q = '''
        ...
        (
             ...
             CONSTRAINT uc_header_video UNIQUE (VideoId)
        )
    '''.format(table_name)
...
# Channel
def create_channel_table(table_name: str):
    channel_create_table_q = '''
        CREATE TABLE {0}
        (
             {0}ID INTEGER PRIMARY KEY AUTOINCREMENT,
             ChannelID TEXT NOT NULL,
             ...
             IsFamilyFriendly BIGINT NULL,
             IsPaidMembership BIGINT NULL,
             ChannelKeywords TEXT NULL,
             NumberOfSubscribers BIGINT NOT NULL,
             CreatedDate DATE,
             CreatedDatetime DATETIME,

             CONSTRAINT uc_channel UNIQUE (ChannelID)
        )
    '''.format(table_name)

    c.execute(channel_create_table_q)
    conn.commit()


insert_into_channel_q = '''
    INSERT INTO {0} (
        ChannelID,
        Name,
        Description,
